...
function AddSeriesForm({ onSeriesAdded }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://libraryuni.free.nf/library/Library/Connection/actions/actionForBooks.php?action=addSeries', {
                method: 'POST',
                body: postData,
            });
...
function EditSeriesForm({ series, onFinished, onCancel }) {
    ...
    const [coverImage, setCoverImage] = useState(null);
    const [imagePreview, setImagePreview] = useState(series.Image_background ? `http://libraryuni.free.nf/library/Library/${series.Image_background}` : null);
    const [message, setMessage] = useState('');
    const [submitting, setSubmitting] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://libraryuni.free.nf/library/Library/Connection/actions/actionForBooks.php?action=updateSeries', {
                method: 'POST',
                body: postData,
            });
            const result = await response.json();
...
function HandleSeries({ onCancel }) {
    ...
    const fetchSeries = async () => {
        try {
            const response = await fetch('http://libraryuni.free.nf/library/Library/Connection/actions/actionForBooks.php?action=getAllSeries');
            const result = await response.json();
            if (result.success) {
...
function SeriesList({ series: seriesList, onDelete, onEdit }) {
    ...
    const handleDeleteSeries = async (seriesId) => {
        ...
        try {
            ...
            const response = await fetch('http://libraryuni.free.nf/library/Library/Connection/actions/actionForBooks.php?action=deleteSeries', { method: 'POST', body: postData });
            const result = await response.json();
