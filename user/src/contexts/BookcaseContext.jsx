...
export const BookcaseProvider = ({ children }) => {
    ...
    const fetchBookcase = useCallback(async () => { 
        ...
        setLoading(true);
        try {
            const response = await fetch(`https://libraryuni.free.nf/library/Library/Connection/actions/actionForMyList.php?action=getListByStudent&StudentID=${user.StudentID}`);
    ...
    const deleteFromList = async (listId) => {
        ...
        try {
            const response = await fetch('https://libraryuni.free.nf/library/Library/Connection/actions/actionForMyList.php?action=deleteFromList', {
                method: 'POST',
                body: postData,
            });
            const result = await response.json();
